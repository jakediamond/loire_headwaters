...
# Some data cleaning
df_w <- df_w %>%
  mutate_at(vars(15:20), funs(if_else(. < 0, NA_real_, .))) # get rid of negative values

# Get rid of crazy values
df_w <- df_w %>%
  mutate(DO = if_else(DO > 20, NA_real_, DO),
         DO_temp = if_else(DO_temp > 35, NA_real_, DO_temp),
         ...
         lux_water_temp = if_else(lux_water_temp > 35, NA_real_, lux_water_temp)) #crazy values out
