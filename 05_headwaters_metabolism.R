...
df <- readRDS("Headwaters/Data/headwaters_data_clean") %>%
  select(datetime, DO.obs = DO, temp.water = DO_temp,
         site = Site, site_code, subwatershed = Subwatershed, discharge = q_mmd)
# Load geometry data
df_k <- read_xlsx("Headwaters/Data/hydraulic_data.xlsx") %>%
  select(site, depth = depth_m)

# Read in watershed info
ws_meta <- read_excel("Headwaters/Data/Loire_headwaters_data_share.xlsx",
                      sheet = "watershed_meta") %>%
  select(watershed, subwatershed, site, latitude, longitude, area = area_km2)

# Overall metadata
df <- left_join(df, ws_meta) %>%
  left_join(df_k)
...
# Calculate depth
df$discharge <- df$discharge * (df$area * 1000^2) / 86400 / 1000
df$depth <- calc_depth(df$discharge)
...
mm_all_mle <- df_n %>%
  # filter(site_code == "coi071") %>%
  # unnest(cols = c(data)) %>%
  # ungroup() %>%
  # select(-site,-site_code,-watershed,-subwatershed) %>%
  # slice(401:600)
  ...
  # # Set the specifications
  # specs <- specs(model_name = mod)
  # 
  # # Do the metabolism
  # met = metab(specs = specs, 
  #       data = as.data.frame(mm_all_mle))
  # met
  # plot_metab_preds(met)
  # plot_DO_preds(met)
  
  
saveRDS(mm_all_mle, "Headwaters/Data/headwaters_metabolism_mle_new")
mm_all_mle <- readRDS("Headwaters/Data/headwaters_metabolism_mle_new")
# Inspect the model -------------------------------------------------------
mm_mle <- mm_all_mle %>%
  mutate(met = map(mm, predict_metab)) %>%
  unnest(met)
saveRDS(mm_mle, "Headwaters/Data/headwaters_metabolism_mle_preds_new")
